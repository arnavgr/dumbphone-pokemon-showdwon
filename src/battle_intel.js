...
function estimateDamage({ level, power, attack, defense, stab, typeMult, weatherMult, burnPenalty }) {
  ...
  return { min: formatDamage(base * 0.85), max: formatDamage(base) };
}
...
function speedComparison(state, dex) {
  ...
  const ownAdj = adjustedSpeed(ownSpeed, { spe: ownBoost });
  const oppAdj = adjustedSpeed(oppPred, { spe: oppBoost });
...
function renderDossier(state) {
  ...
  for (const e of entries) {
    ...
    const speed = active && normalizeName(active.species) === normalizeName(e.species) && active.predictedSpeed != null
...
async function renderDamageEstimator(state, dex, moves) {
  ...
  for (const m of candidateMoves) {
    ...
    const dmg = estimateDamage({ level, power: Number(data.basePower), attack: Number(atkStat), defense: Number(targetDef), stab, typeMult, weatherMult: w, burnPenalty: burned });
    if (!dmg) continue;
    ...
    const ko = hitsBest === 1 ? (hitsWorst === 1 ? "guaranteed OHKO" : "possible OHKO") : `${hitsBest}-${hitsWorst} hit KO`;
    body += `<div><strong>${esc(m.move)}</strong> [${esc(data.type)} / ${esc(category)}] ~${dmg.min}-${dmg.max} damage (${pctMin}-${pctMax}%) — ${esc(ko)}</div>`;
  }
  return body;
}
...
export async function renderEnhancedBattle(state) {
  ...
  if (req?.active?.length) {
    ...
    movesForTurn.forEach((m, i) => {
      if (!m.disabled) body += `<p><a href="/choose?value=${encodeURIComponent(`move ${i + 1}`)}">${i + 1}. ${esc(m.move)}</a></p>`;
    });
  }
  return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(state.roomTitle || "Battle")}</title></head><body>${body}</body></html>`;
}
